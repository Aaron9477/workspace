...
int main(int argc, char * argv[]){
 ...
 // if(ros_enable){
  ...
  ros::NodeHandle n;
  ros::Publisher tld_pub = n.advertise<sensor_msgs::RegionOfInterest>("tld_tracking", 1000);
...
REPEAT:
  while(capture.read(frame)){
	...
	if (ros_enable){
		...
		ROS_INFO("tld published, the first point is %f,%f, the second point is %f,%f",msg_pub.x_offset,msg_pub.y_offset,msg_pub.height,msg_pub.width); 	
