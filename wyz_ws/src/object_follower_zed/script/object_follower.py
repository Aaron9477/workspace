...
from object_follower_zed.msg import zed_roi
...
class ObjectFollower():
    def __init__(self):
        ...
        rospy.loginfo("Waiting for zed_roi topic...")
        
        rospy.wait_for_message('/zed_roi', zed_roi)
                    
        # Subscribe to the depth image
        # self.depth_subscriber = rospy.Subscriber("/zed_roi", zed_roi, self.convert_depth_image, queue_size=1)
        
        # Subscribe to the ROI topic and set the callback to update the robot's motion
        rospy.Subscriber('/zed_roi', zed_roi, self.set_cmd_vel)
    ...
    def set_cmd_vel(self, msg):
        # Acquire a lock while we're setting the robot speeds
        print "I got message from zed_tracker, x_offset:{a}, y_offset:{b}, height:{c}, width:{d}, depth:{e}".format(a=msg.x_offset, b=msg.y_offset, c=msg.height, d=msg.width, e=msg.depth)
