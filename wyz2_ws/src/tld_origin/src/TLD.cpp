...
void TLD::init(const Mat& frame1,const Rect& box,FILE* bb_file){
  ...
  //Get Bounding Boxes
    buildGrid(frame1,box); //finish examination_WYZ
...
void TLD::getPattern(const Mat& img, Mat& pattern,Scalar& mean,Scalar& stdev){
  ...
  pattern.cols = 15;
  pattern.rows = 15;
  cv::resize(img,pattern,Size(patch_size,patch_size), 0, 0, CV_INTER_AREA);////////////////////////////////////////////////////////////////////////////
  //cv::resize(img,pattern,Size(15,15));
...
void TLD::buildGrid(const cv::Mat& img, const cv::Rect& box){
  ...
  double step = 0; //////////////////////////////
  if (round(SHIFT*min_bb_side) <= 0) step = 1; ///////////////////////////
  else step = round(SHIFT*min_bb_side); //////////////////////////////////
